    ...
    <div className="page-container page-enter">
          ...
          {[...Array(5)].map((_, i) => (
            <div className="stat-card" key={i}>
              <div className="stat-header">
                <div style={{ flex: 1 }}>
                  <Skeleton type="text" lines={2} lineHeight="14px" />
                </div>
                <Skeleton.Circle size="40px" />
              </div>
              <Skeleton.Box style={{ height: "40px", width: "100%", marginTop: "12px" }} />
            </div>
          ))}
        </div>
      ) : (
      ...
      <div className="card hover-lift">
        <div className="filters-card">
          ...
          <button className="ghost-btn" onClick={clearFilters}>Reset</button>
        </div>

        {loading ? (
          <div className="table-wrap">
            <Skeleton.Table rows={8} cols={10} />
          </div>
        ) : filtered.length === 0 ? (
