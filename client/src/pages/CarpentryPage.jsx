    ...
    <div className="page-container page-enter">
        ...
        <div className="stats-grid">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="stat-card">
              <Skeleton.Box w="60%" h="14px" />
              <Skeleton.Box w="40%" h="28px" />
              <Skeleton.Box w="80%" h="12px" />
            </div>
          ))}
        </div>
      ) : (
      ...
      <div className="card hover-lift">
        <div className="filters-card">
          ...
          <button className="ghost-btn" onClick={clearFilters}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 6 }}><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg> Reset</button>
        </div>

        <div className="table-wrap">
          {loading ? (
            <Skeleton.Table rows={5} cols={9} />
          ) : paged.length === 0 ? (
            <EmptyState
              icon={<Hammer size={32} />}
              title="No carpentry items found"
              description="Try adjusting your search or filters"
              action={<button className="primary" onClick={clearFilters}>Clear Filters</button>}
            />
          ) : (
            <table>
              <thead>
