...
import { getAll, createOne, getOne, updateOne, remove } from "../controllers/supplier.controller.js";

const router = Router();
router.get("/", async (req, res, next) => { try { res.json(await getAll()); } catch (err) { next(err); } });
router.post("/", async (req, res, next) => { try { res.status(201).json(await createOne(req.body)); } catch (err) { if (err.code === 11000) return res.status(409).json({ message: "Supplier already exists" }); next(err); } });
